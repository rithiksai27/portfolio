...
export const GlareCard4 = ({
  ...
  const normalBackgroundStyle = {
    "--custom-image-1": `url("https://s3.amazonaws.com/coursera_assets/meta_images/generated/CERTIFICATE_LANDING_PAGE/CERTIFICATE_LANDING_PAGE~J4XR9UWHANQS/CERTIFICATE_LANDING_PAGE~J4XR9UWHANQS.jpeg")`,
    "--pattern": "var(--custom-image-1) center/cover no-repeat",
    "--rainbow": "",
    "--diagonal": "",
    "--shade": "",
  ...
  const hoverBackgroundStyle = {
    "--pattern": isHovered ? "var(--custom-image-2) center/cover no-repeat" : "var(--custom-image-1) center/cover no-repeat",
  } as React.CSSProperties;
    ...
    >
      <div className="h-full grid will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] rounded-[var(--radius)] border border-slate-800 hover:[--opacity:0.6] hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none overflow-hidden">
        <div className="w-full h-full grid [grid-area:1/1] mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))]">
          <div className={cn("h-full w-full bg-slate-950", className)} style={isHovered ? hoverBackgroundStyle : normalBackgroundStyle}>
            {children}
          </div>
        </div>
        ...
        <div
          className="w-full h-full grid [grid-area:1/1] mix-blend-color-dodge opacity-[var(--opacity)] will-change-background transition-opacity [clip-path:inset(0_0_1px_0_round_var(--radius))] [background-blend-mode:hue_hue_hue_overlay] [background:var(--pattern)] relative after:content-[''] after:grid-area-[inherit] after:bg-repeat-[inherit] after:bg-attachment-[inherit] after:bg-origin-[inherit] after:bg-clip-[inherit] after:bg-[inherit] after:mix-blend-exclusion after:[background-size:var(--foil-size),_cover] after:[background-position:center]"
          style={{ ...normalBackgroundStyle }}
        />
      </div>
    </div>
  );
